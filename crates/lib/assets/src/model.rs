...
fn create_unlit_material(
    ...
    let base = if let Some(texture) = material.pbr_metallic_roughness().base_color_texture() {
        process_texture(ctx, &texture.texture(), ImagePurpose::Color)
    } else {
        process_placeholder(
            ctx,
...
fn create_pbr_material(
    ...
    {
        process_texture(ctx, &texture.texture(), ImagePurpose::NonColor)
    } else {
        process_placeholder(
            ctx,
            [
    ...
    let occlusion = if let Some(texture) = material.occlusion_texture() {
        ...
        process_placeholder(ctx, [0.0, 0.0, 1.0, 1.0], ImagePurpose::NonColor)
    };

    let normal = if let Some(texture) = material.normal_texture() {
        process_texture(ctx, &texture.texture(), ImagePurpose::Normals)
    } else {
        process_placeholder(ctx, [0.0, 0.0, 1.0, 1.0], ImagePurpose::Normals)
    };
...
pub fn process_model(gltf: GltfContent) -> ModelAsset {

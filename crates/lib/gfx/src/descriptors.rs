...
use crate::{BufferHandle, GpuDescriptorSet, ImageHandle, ResourceManager};

const BASIC_DESCIPTOR_UPDATE_COUNT: usize = 512;

/// Allows client to manipulate descriptor set
impl<'a> UpdateDescriptorSetsContext<'a> {
    pub fn bind_storage_image(
        ...
        if let Some(point) = image_bind {
            point.data = Some((
                image,
                data.get_or_create_view(&self.resource_manager.raw, ImageViewDesc::color())?, // FIXME: even for depth?
                layout.into(),
            ));
    ...
    pub fn bind_image(
        ...
        if let Some(point) = image_bind {
            point.data = Some((
                image,
                data.get_or_create_view(&self.resource_manager.raw, ImageViewDesc::color())?,
                layout.into(),
            ));
            self.dirty.insert(handle);
        }
...
impl<'a> Drop for UpdateDescriptorSetsContext<'a> {
    fn drop(&mut self) {
        let mut drop_list = self.resource_manager.current_drop_list.lock();
...
impl Device {
    ...
    where
        F: FnOnce(&mut UpdateDescriptorSetsContext) -> BackendResult<()>,
    {
        let mut context = UpdateDescriptorSetsContext {
    ...
    fn is_valid_descriptor_impl(
        container: &HotColdPool<vk::DescriptorSet, Box<DescriptorSetData>>,
        handle: DescriptorHandle,
    ) -> bool {
    ...
    pub fn create_bind_group(&self, desc: &BindGroupLayoutDesc) -> BackendResult<DescriptorHandle> {
    ...
    pub fn destroy_bind_group(&self, handle: DescriptorHandle) {
